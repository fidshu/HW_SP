...
try:
    from SimFeatUp.featup.adaptive_conv_cuda.adaptive_conv import AdaptiveConv
except:
    AdaptiveConv = None

def adaptive_conv_py_simple(input, filters):
    b, c, h1, w1 = input.shape # (1, 512, 34, 34)
    b, h2, w2, f1, f2 = filters.shape # (1, 28, 28, 7, 7)
    assert f1 == f2
    kernel_size = f1

    t_filters = filters.reshape(b, h2, w2, f1 * f2) # [1, 28, 28, 49]

    patches = torch.nn.Unfold(kernel_size)(input) \
        .view((b, c, f1 * f2, h2, w2)) # [1, 512, 49, 28, 28]
...
class JBULearnedRange(torch.nn.Module):
    ...
    def get_range_kernel(self, x):
        GB, GC, GH, GW = x.shape
        proj_x = self.range_proj(x) # [1, key_dim, 28, 28]
        proj_x_padded = F.pad(proj_x, pad=[self.radius] * 4, mode='reflect') # [1, key_dim, 34, 34]
        queries = torch.nn.Unfold(self.diameter)(proj_x_padded) \
            .reshape((GB, self.key_dim, self.diameter * self.diameter, GH, GW)) \
            .permute(0, 1, 3, 4, 2) # [1, key_dim, 28, 28, 49]
    ...
    def forward(self, source, guidance):
        GB, GC, GH, GW = guidance.shape # [1, 3, 28, 28]
        SB, SC, SH, SQ = source.shape # [1, 512, 14, 14]
        assert (SB == GB)

        spatial_kernel = self.get_spatial_kernel(source.device) # [1, 49, 1, 1]
        range_kernel = self.get_range_kernel(guidance) # [1, 49, 28, 28]
        ...
        combined_kernel = combined_kernel.permute(0, 2, 3, 1) \
            .reshape(GB, GH, GW, self.diameter, self.diameter) # [1, 28, 28, 7, 7]
...
class JBUOne(torch.nn.Module):
    ...
    def forward(self, source, guidance):
        ...
        return self.fixup_proj(source_2) * 0.1 + source_2
