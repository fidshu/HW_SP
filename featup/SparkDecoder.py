...
class SparseConvNeXtLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        if x.ndim == 4: # BHWC or BCHW
            ...
            if self.sparse:
                # return nn.LayerNorm(x.shape[2]).to('cuda:0').forward(x)
                raise NotImplementedError
            else:
                return super(SparseConvNeXtLayerNorm, self).forward(x)
...
class SparseDecoder(nn.Module):
    def __init__(self, cnn, sbn=False, verbose=False):
        super(SparseDecoder, self).__init__()
        self.sp_cnn = SparseDecoder.dense_model_to_sparse(m=cnn, verbose=verbose, sbn=sbn)
    ...
    def dense_model_to_sparse(m: nn.Module, verbose=False, sbn=False):
        ...
        for name, child in m.named_children():
            # print(name)
            # if(name == 'SelfAttn' or name =='cw'or name =='ch'or name =='cat'or name =='ca'):
            #     continue;
            oup.add_module(name, SparseDecoder.dense_model_to_sparse(child, verbose=verbose, sbn=sbn))
        del m
        return oup
    
    def forward(self, x, gudience):
        return self.sp_cnn(x,gudience)
