...
from featup.SparkDecoder import SparseDecoder
import featup.SparkDecoder as SparkDecoder
torch.multiprocessing.set_sharing_strategy('file_system')


# import os
# os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 只让程序看到 GPU 1
...
class SimFeatUp(pl.LightningModule):
    def __init__(self,
        ...
        # state = torch.load("/mnt/sda/pai-hdd/bknian/CLIP/SegEarth-OV_down/simfeatup_dev/weights/xclip_jbu_one_million_aid.ckpt", map_location='cpu')["state_dict"]
        state = torch.load("/mnt/sda/pai-hdd/bknian/CLIP/SimFeatUp-main/work_dirs/simfeatup_million_aid/checkpoints/jbu_one/xclip_jbu_one_million_aid_attention_crf_0_tv_0.0_ent_0.0.ckpt", map_location='cpu')["state_dict"]
        ...
        if self.MAE:
            ...
            for key, value in state_dict.items():
                if key.startswith('upsampler.'):
                    new_key = key.replace('upsampler.', '')  # 移除upsampler.前缀
                    corrected_state_dict[new_key] = value
                else:
                    corrected_state_dict[key] = value
            
            # 使用处理后的state_dict加载
            self.upsampler.sp_cnn.load_state_dict(corrected_state_dict, strict=True)
            print("--------------预训练模型加载成功-----------------")
        else:
            # 处理键名不匹配的问题
            state_dict = state.get('module', state)
            
            # 创建新的state_dict，移除upsampler.sp_cnn.前缀
            new_state_dict = {}
            for key, value in state_dict.items():
                if key.startswith('upsampler.sp_cnn.'):
                    new_key = key.replace('upsampler.sp_cnn.', '')
                    new_state_dict[new_key] = value
                else:
                    new_state_dict[key] = value
            
            self.upsampler.load_state_dict(new_state_dict, strict=True)
            print("--------------预训练模型加载成功-----------------")
    ...
    def forward(self, x):
        if(self.MAE):
            active_b1ff,mask = self.mask(x.shape[0], x.shape[2], x.shape[3], x.device)
            ...
            masked_bchw = feature_encoder * mask

            feature_encoder = self.model(masked_bchw)
        else:
            feature_encoder = self.model(x)

        return self.upsampler(feature_encoder)
        # if(self.MAE):
        #     active_b1ff,mask = self.mask(feature_encoder.shape[0], feature_encoder.shape[2], feature_encoder.shape[3], x.device)
        ...
        #     masked_bchw = feature_encoder * mask
        #     return self.upsampler(masked_bchw)
        # else:
        #     return self.upsampler(feature_encoder)
...
def my_app(cfg: DictConfig) -> None:
    ...
    loader = DataLoader(
        subset_dataset , cfg.batch_size, shuffle=True, num_workers=cfg.num_workers)
    
    val_loader = DataLoader(
        SingleImageDataset(0, subset_dataset , 1), 1, shuffle=False, num_workers=cfg.num_workers)
    ...
    progress_bar = CustomTQDMProgressBar(refresh_rate=10)
    trainer = Trainer(
        accelerator='gpu',
        strategy="ddp",
