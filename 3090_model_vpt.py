...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dense=False, use_mae = False):
        if(use_mae):
            ...
            if(random_value == 1):
                ...
                blur_bchw = x * active_b1hw.float() + self.create_gaussian_blur(x,kernel_size = resolution+1) * (1 - active_b1hw.float())
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, masks=None, pool_mask=None, dense=False,use_mae=False):
        if pool_mask is not None:
            return self.visual(image.type(self.dtype), mask=pool_mask, dense=dense,use_mae=use_mae)
        if masks == None:
            return self.visual(image.type(self.dtype), dense=dense,use_mae=use_mae)
        else:
            return self.visual(image.type(self.dtype), masks.type(self.dtype),use_mae=use_mae)
